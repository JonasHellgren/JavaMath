...
public class SwingShowHeatMap {
    ...
    @Builder
    record WidthAndHeight(int width, int height, int margin, int nCols, int nRows) {
    ...
    record TextXY(int col, int row, Graphics2D g2d, String text, WidthAndHeight wh) {
        ...
        int textX() {
            return  getX() + (wh.cellWidth() - metrics().stringWidth(text)) / 2;}
        ...
        private int getY() {
            return (wh.nRows - 1 - row) * wh.cellHeight();
        }
